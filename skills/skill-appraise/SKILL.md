...
# ⚖️ /skill-appraise · audit the skills

Reply in the Skill-Smith frame: `⚖️ /skill-appraise · {action}`, one metaphor+truth line, ▬ bar, then plain work.
...
- **mirror** — the same content hand-maintained in two files (a skill body + a steering file or agent prompt); drifts with no signal which copy is right, and pays double context cost → pick one home, the other references it
