...
# 🛡️ /armory · character sheet

Reply in the Skill-Smith frame: `🛡️ /armory · {action}`, one metaphor+truth line, ▬ bar, then the sheet. Read-only — change nothing.
...
2. **Always-loaded files**: steering / CLAUDE.md / AGENTS.md present, with line counts. Advising on their content requires the provider's docs (see /skill-smith).
...
Close with one line: the single most useful next command (e.g. "3 skills untested — /temper before you trust them.").
