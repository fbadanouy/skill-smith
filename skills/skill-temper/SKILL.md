...
# 🗡️ /skill-temper · test the skill

Reply in the Skill-Smith frame: `🗡️ /skill-temper · {action}`, one metaphor+truth line, ▬ bar, then plain work.
...
Run each as cleanly as possible (fresh context, only the skill names + descriptions visible, ask: "which skill, if any, handles this?"). Run each prompt 2–3× and score a trigger rate — pass at a clear majority, not a single lucky hit. If the harness has a headless/non-interactive mode, prefer a real run over the simulated judge (`harness/<platform>/mechanics.md` documents the command if the pack is installed, else fetch the provider's docs; never guess it).
Positive prompt silent → description too narrow → ⚡ /skill-sharpen. Near-miss triggers → too broad → ⚡ /skill-sharpen.

## 3 — it behaves

Per scenario (from the original request, the failure that caused a /skill-repair, or /skill-forge's watch-it-fail baseline record — that failure is always test #0):
...
Table: test · expected · actual · PASS/FAIL · evidence. Evidence quotes the output — no PASS on benefit of the doubt (a "Summary" heading over one vague sentence is a FAIL). Report cost beside the delta: tokens/turns with vs without — a skill that triples the cost for a marginal gain fails even on all-pass. Ship only on all-pass. Record in `ledger.md`.
