# ⚒️ SKILL-SMITH

### **local only. stateless skills. craft the ultimate skill.**

[![standard: agentskills.io](https://img.shields.io/badge/standard-agentskills.io-8b5a2b)](https://agentskills.io)
![dependencies: none](https://img.shields.io/badge/dependencies-none-2ea44f)
![size: ~400 lines](https://img.shields.io/badge/size-~400_lines-blue)
![telemetry: zero](https://img.shields.io/badge/telemetry-zero-black)
![install: cp -R](https://img.shields.io/badge/install-cp%20--R-orange)

![The Skill-Smith at work](skill-smith.gif)

```
═══════════════════════════════════════════════════════════
  A dark room. Coals breathe orange.
  An anvil. A ledger. Seven hammers.
  The SMITH does not look up.

  "State your business."

  > _
═══════════════════════════════════════════════════════════
```

Skills are markdown. Small scrolls. They teach your agent to fight.

Most scrolls at market are cursed. Forged in one strike. Never tested.
Dead weight in your pack.

The SMITH forges true. Tests every blade against bare hands.
If your agent wins bare-handed, the SMITH refuses your gold.

## THE EIGHT HAMMERS

```
  ⚒️  /skill-forge      new skill. baseline first, or no deal
  🔥  /skill-smelt      melt raw docs down to skills
  ⚡  /skill-sharpen    dull edge activates late. fix it
  🔧  /skill-repair     blade bit its wielder. find the cursed line
  🗡️  /skill-temper     prove it: loads, activates, beats bare hands
  ⚖️  /skill-appraise   whole pack on the anvil. judged
  🛡️  /skill-armory     character sheet. equipped. weight.
  🪖  /skill-smith-fit  armor, not blade: steering, hooks, agents

  🧙  /skill-smith      just speak. the SMITH picks the hammer
```

## THE HARNESS

A blade wins duels. A harness wins wars.
The SMITH measures the wearer before he forges.

`harness/<platform>/` — mechanics, primitives, templates, a docs mirror
with a pull script. Verified against real docs, every claim with receipts.
The hammers read it before they touch platform steel.
Fitted: **kiro** · **codex** · **claude-code**.

## THE OATH

```
  ✗ no forging what bare hands already win
  ✗ no writing facts without a source
  ✗ no touching disk before you approve
  ✗ no reporting tests never run
  ✦ nothing leaves the forge untempered
  ✦ the ledger remembers every strike
```

## ENTER

```
  $ git clone https://github.com/fbadanouy/skill-smith
  $ cd skill-smith && ./install.sh

  > /skill-smith
```

No server. No account. No build. ~400 lines of hammers.
Read every line. Then trust it. That is the point.

## THE TOMES

The SMITH did not invent its craft. It studied — ancient tomes of
eval-loop testing, blind A/B trials, the discipline of the failing
baseline. The knowledge is old. The hammer is new.

```
═══════════════════════════════════════════════════════════
  The SMITH nods once. The coals flare.

  EXITS: [skills/]  [install.sh]  [ledger]
═══════════════════════════════════════════════════════════
```
